...
- **Permissions and Tooling:** Use built-in `write`, authorized by `edit` only for `research/results/*.md`, and built-in `glob`, authorized only for `research/results/*.md`, to perform best-effort collision checks. Do not add the broad `fsrw` MCP: its current workspace-wide mount cannot enforce the required per-directory write boundary. Retain denials for built-in `read`, list, shell, external directories, non-result paths, and unrelated tools. Pre-provision the destination because no directory-creation permission is granted to Librarian. Validate the exact path patterns against the installed OpenCode version before changing the agent.
...
- [ ] **Task 1: Define and document the research artifact contract**
  ...
  - **Review Criteria:** The destination exists in clean checkouts of both repositories, documentation distinguishes canonical documentation from workspace-owned artifacts, required fields and allowed `status` values are stated, names are single path segments matching the permission rule, symlinked destinations are rejected, and generated artifacts are not ignored by Git.
- [ ] **Task 2: Give Librarian least-privilege persistence access**
  - **Description:** First run a minimal OpenCode permission probe against the installed version to confirm that `edit` gates `write` and that `research/results/*.md` rules match only direct Markdown children. Then update `agent-harness/.opencode/agents/Librarian.md` to allow `edit` and `glob` only for that pattern. Preserve web access and all unrelated denials. Do not grant `read`, list, `bash`, unrestricted MCP filesystem tools, recursive result access, or `external_directory` access.
  - **Review Criteria:** The recorded probe proves Librarian can glob/create a direct result Markdown file while attempts against `README.md`, `research/other.md`, `research/results/nested/file.md`, and paths containing `..` are denied. Configuration parses under the active OpenCode version.
- [ ] **Task 3: Add persistence to the Librarian workflow**
  ...
  - **Review Criteria:** Direct Librarian invocations—not only `/research`—produce one conforming artifact; untrusted source text cannot add frontmatter keys; source URLs/version context are retained; detected collisions do not overwrite; failed writes report the attempted path and tool error; partial research plus write failure reports both conditions.
- [ ] **Task 4: Align the research command and harness documentation**
  - **Description:** Update `agent-harness/.opencode/commands/research.md` to require the Librarian's `Research artifact: ...` line, pass that path through in its final summary, and never write a second research artifact; Builder continues generating only `SKILL.md`. If the line is absent or persistence failed, Builder stops before skill generation and surfaces the Librarian error. Update `agent-harness/README.md` and setup documentation to describe destination provisioning and narrowly scoped local persistence.
  - **Review Criteria:** `/research` still produces its skill after a successful handoff, cites the persisted artifact, and writes exactly one result per Librarian call; a missing/failed handoff aborts visibly; public permission and setup documentation match configuration.
- [ ] **Task 5: Add repeatable contract and permission validation**
  - **Description:** Add repository-native static validation for agent permission boundaries/instructions and representative complete, partial, malformed, YAML-injection, traversal-name, and secret-like fixtures. Add executable OpenCode integration smoke tests (or a documented command matrix if agent execution cannot be automated) covering direct invocation, `/research` handoff, missing destination, symlinked destination, detected collision, partial source failure, persistence denial, partial-plus-persistence failure, and denied writes outside the destination. State explicitly that prompt checks reduce but cannot prove absence of secrets or guarantee every future model invocation persists.
  - **Review Criteria:** Static validation exits nonzero with actionable errors for malformed artifacts or widened permissions. Integration evidence demonstrates successful direct and `/research` paths plus all negative/error paths; limitations are documented rather than represented as hard guarantees; no new third-party dependency is required.
- [ ] **Task 6: Synchronize and verify the canonical change**
  - **Description:** Run the existing harness synchronization workflow so only canonical `.opencode` changes propagate to the root project copy. Provision root `research/results/` separately. Verify diffs, validation checks, and sync status before review; do not add `research/results` to `.harness-sync` because project research must remain local to its invoking workspace.
  - **Review Criteria:** Root `.opencode` reflects canonical agent/command updates, each repository has its independently tracked destination placeholder, `harness-sync status` reports no unintended drift, tests pass, and no generated artifact crosses workspace boundaries.
